...
const fs = require('fs');//library that build in into the node. If we have an error saving a book we want to be sure that we removed the bookcover from db. fs -file sysem
...
const upload = multer({
  ...
  fileFilter: (req, file, callback) => {
    callback(null, imageMimeTypes.includes(file.mimetype));//to filter the files that the server can accept. We need callback after we done with a file. null means we don't have any error. We need boolean to determine if the file is accepted (true) or not (faulse)
  }
...
//New Book Router. This route is for displaying the form where you can add an author
router.get('/new', async (req, res) =>{//in the result it will be authors/new
renderNewPage(res, new Book());//we're passing new book here and there won't be any error
});
//Create Book Route
//set up the route to accept files. The file name is cover (the name you set on your input).It creates file, upload on a server and put it in a correct folder. This library is going to add a variable to our request which is called file. req.file is going to be a file that we're uploading to our server
router.post('/', upload.single('cover'),//the name of the input is cover so we specify it here. It tells multer that we're going to upload a single file called cover. It going to do all the work behind the sceine: to create this file, upload it on the server and put it into correct folder. This library is going to add a variable to our request here which is called "file"(req.file)
 async (req, res) => {//asyncronise function
  ...
  const book = new Book({
    ...
    puplishDate: new Date(req.body.publishDate),
    pageCount: req.body.pageCount,
    coverImageName: fileName,//if we uploaded the file it will be equal to the name of the file  if we didn't upload the file it will be equal to null so we can send an error
